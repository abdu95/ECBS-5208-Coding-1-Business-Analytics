...
#lowes non-parametric regression
...
# non-parametric regressions are pretty volatile: this is not feature of data

# You can change the scale for Total GDP and life-expectancy for checking log-transformation
ggplot( df , aes(x = gdptot, y = lifeexp ))  +
  geom_point() +
  geom_smooth(method="loess")+
  labs(x = "Total GDP (2018 int. const. $, PPP , ln scale )",y = "Life expectancy  (years, ln scale)") +
  scale_x_continuous( trans = log_trans(),  breaks = c(1,2,5,10,20,50,100,200,500,1000,10000) )+
...
ggplot( df , aes(x = gdppc, y = lifeexp ))  +
  geom_point() +
  geom_smooth(method="loess")+
  labs(x = "GDP/capita (2018 int. const. $, PPP , ln scale )",y = "Life expectancy  (years, ln scale)") +
  scale_x_continuous( trans = log_trans(), breaks = seq(0, 120, by = 20))
  scale_y_continuous( trans = log_trans() )
...
#     this is piecewise linear spline. with knot
#   Extra: weighted-ols:
...
poly(df$ln_gdppc, 2)
poly(df$ln_gdppc, 2, raw = TRUE)
# Do the regressions
#
# Built in regression in R
# lm- linear model
# 1st arg: function
# ~ tilda means y is regressed by 
reg_b <- lm( lifeexp ~ ln_gdptot , data = df )
reg_b
summary( reg_b )

# residuals (errors) have 0 mean, thats why mean is not shows 
...
# HC2 certain type of heteroskadastic Error 
...
#convex. sign of Beta2 is positive. 
...
# R ^2 bigger = 0.7
# variance bigger in some ppoints
...
# multiple cutoff: cutoff<- c(50, 100, 150)
cutoff<- 50
